...
function Fourfeetbed({response}){
  ...
  const [size, setSize] = useState("4FT");
  ...
  useEffect(() => {
    ...
    router.push(`/divanbed/four-feet-bed?${size ? "size=" + size : ""}${maxPrice ? size ? "&maxPrice=" + maxPrice : "maxPrice=" + maxPrice : ""}${minPrice ? "&minPrice=" + minPrice : ""}${images ? "&color=" + images : ""}`)
                  ...
                  >
                    <select className="dropdown-section"
                      onChange={(e) => {
                        ...
                        setMinPrice(value.minPrice);
                        router.push(`/divanbed/four-feet-bed?maxPrice=${value.maxPrice}&minPrice=${value.minPrice}${router?.query?.size ? "&size=" + router.query.size : ""}&color=${value.images}`)
