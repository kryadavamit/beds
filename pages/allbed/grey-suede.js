...
function greySuede({response}){
  ...
  useEffect(() => {
    ...
    router.push(`/allbed/grey-suede?${size ? "size=" + size : ""}${maxPrice ? size ? "&maxPrice=" + maxPrice : "maxPrice=" + maxPrice : ""}${minPrice ? "&minPrice=" + minPrice : ""}${images ? "&color=" + images : ""}`)
                  ...
                  >
                    <select className="dropdown-section"
                      onChange={(e) => {
                        ...
                        setMinPrice(value.minPrice);
                        router.push(`/allbed/grey-suede?maxPrice=${value.maxPrice}&minPrice=${value.minPrice}${router?.query?.size ? "&size=" + router.query.size : ""}&color=${value.images}`)
...
<div className="row">
                {response.map((item) => {
                  ...
                  if(item.category=='Ottoman Beds'){
                    ...
                    return (
                      <OttomanbedBox
                        src={item.images[0].color2.base_url}
