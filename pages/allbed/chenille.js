...
function Linenfabric({response}){
  ...
  useEffect(() => {
    ...
    router.push(`/allbed/chenille?${size ? "size=" + size : ""}${maxPrice ? size ? "&maxPrice=" + maxPrice : "maxPrice=" + maxPrice : ""}${minPrice ? "&minPrice=" + minPrice : ""}${images ? "&color=" + images : ""}`)
                  ...
                  >
                    <select className="dropdown-section"
                      onChange={(e) => {
                        ...
                        setMinPrice(value.minPrice);
                        router.push(`/allbed/chenille?maxPrice=${value.maxPrice}&minPrice=${value.minPrice}${router?.query?.size ? "&size=" + router.query.size : ""}&color=${value.images}`)
...
<div className="row">
                {response.map((item) => {
                  ...
                  if(item.category=='Ottoman Beds'){
                    ...
                    return (
                      <OttomanbedBox
                        src={item.images[0].color3.base_url}
                        title={item.product_name}
                        price={item.price}
                        category={item.category}
                        size={item.size}
                        color="Charcoal Chenille"
                      />
