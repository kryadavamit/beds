...
function BlackCrushed({response}){
  ...
  useEffect(() => {
    ...
    router.push(`/allbed/black-crushed?${size ? "size=" + size : ""}${maxPrice ? size ? "&maxPrice=" + maxPrice : "maxPrice=" + maxPrice : ""}${minPrice ? "&minPrice=" + minPrice : ""}${images ? "&color=" + images : ""}`)
                  ...
                  >
                    <select className="dropdown-section"
                      onChange={(e) => {
                        ...
                        setMinPrice(value.minPrice);
                        router.push(`/allbed/black-crushed?maxPrice=${value.maxPrice}&minPrice=${value.minPrice}${router?.query?.size ? "&size=" + router.query.size : ""}&color=${value.images}`)
...
<div className="row">
                {response.map((item) => {
                  ...
                  if(item.category=='Ottoman Beds'){
                    ...
                    return (
                      <OttomanbedBox
                        src={item.images[0].color4.base_url}
...
export async function getServerSideProps(context) {
  ...
  const data = await axios.get(
    `${process.env.BASE_URL}/api/allbeds/allbeds-productApi?size=${size}&minPrice=${minPrice}&maxPrice=${maxPrice}`,
    // {
    //   method: "color",
    //   value: "color2",
    // }
